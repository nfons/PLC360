#!/usr/bin/python
#
# exp.py - Classes to represent underlying data structures for the grammar
#	 below, for the mini-compiler.
#
# Kurt Schmidt
# 8/07
# Modified Nate Fonseka
# 8/11
# DESCRIPTION:
#		Just a translation of the C++ implementation by Jeremy Johnson (see
#		programext.cpp)
#
# EDITOR: cols=80, tabstop=2
#
# NOTES
#	environment:
#		a dict
#
#		Procedure calls get their own environment, can not modify enclosing env
#
#	Grammar:
#		program: stmt_list 
#		stmt_list:  stmt ';' stmt_list 
#		    |   stmt  
#		stmt:  assign_stmt 
#		    |  define_stmt 
#		    |  if_stmt 
#		    |  while_stmt 
#		assign_stmt: IDENT ASSIGNOP expr
#		define_stmt: DEFINE IDENT PROC '(' param_list ')' stmt_list END
#		if_stmt: IF expr THEN stmt_list ELSE stmt_list FI
#		while_stmt: WHILE expr DO stmt_list OD
#		param_list: IDENT ',' param_list 
#		    |      IDENT 
#		expr: expr '+' term   
#		    | expr '-' term   
#		    | term            
#		term: term '*' factor   
#		    | factor            
#		factor:     '(' expr ')'  
#		    |       NUMBER 
#		    |       IDENT 
#		    |       funcall 
#		funcall:  IDENT '(' expr_list ')'
#		expr_list: expr ',' expr_list 
#		    |      expr 
#

import sys

####  CONSTANTS   ################

	# the variable name used to store a proc's return value
returnSymbol = 'return'

tabstop = '  ' # 2 spaces

######   CLASSES   ##################

class Expr :
	'''Virtual base class for expressions in the language'''

	def __init__( self ) :
		raise NotImplementedError(
			'Expr: pure virtual base class.  Do not instantiate' )

	def eval( self, nt, ft ) :
		'''Given an environment and a function table, evaluates the expression,
		returns the value of the expression (an int in this grammar)'''

		raise NotImplementedError(
			'Expr.eval: virtual method.  Must be overridden.' )

	def display( self, nt, ft, depth=0 ) :
		'For debugging.'
		raise NotImplementedError(
			'Expr.display: virtual method.  Must be overridden.' )

class Number( Expr ) :
	'''Just integers'''

	def __init__( self, v=0 ) :
		self.value = v
	
	def eval( self, nt, ft ) :
		return self.value

	def display( self, nt, ft, depth=0 ) :
		print "%s%i" % (tabstop*depth, self.value)

class Ident( Expr ) :
	'''Stores the symbol'''

	def __init__( self, name ) :
		self.name = name
	
	def eval( self, nt, ft ) :
		return nt[ self.name ]

	def display( self, nt, ft, depth=0 ) :
		print "%s%s" % (tabstop*depth, self.name)

''' BELOW ARE +  - * / defines '''
class Plus( Expr ) :
	'''+ expression '''

	def __init__( self, lhs, rhs ) :
		self.lhs = lhs
		self.rhs = rhs
	
	def eval( self, nt, ft ) :
		return self.lhs.eval( nt, ft ) + self.rhs.eval( nt, ft )

	def display( self, nt, ft, depth=0 ) :
		print "%sADD" % (tabstop*depth)
		self.lhs.display( nt, ft, depth+1 )
		self.rhs.display( nt, ft, depth+1 )
		

class Minus( Expr ) :
	'''- subtraction'''

	def __init__( self, lhs, rhs ) :
		self.lhs = lhs
		self.rhs = rhs
	
	def eval( self, nt, ft ) :
		return self.lhs.eval( nt, ft ) - self.rhs.eval( nt, ft )

	def display( self, nt, ft, depth=0 ) :
		print "%sSUB" % (tabstop*depth)
		self.lhs.display( nt, ft, depth+1 )
		self.rhs.display( nt, ft, depth+1 )
		

class Times( Expr ) :
	'''expression for binary multiplication'''

	def __init__( self, lhs, rhs ) :
		'''lhs, rhs are Expr's, the operands'''

		# test type here?
		# if type( lhs ) == type( Expr ) :
		self.lhs = lhs
		self.rhs = rhs
	
	def eval( self, nt, ft ) :
		return self.lhs.eval( nt, ft ) * self.rhs.eval( nt, ft )

	def display( self, nt, ft, depth=0 ) :
		print "%sMULT" % (tabstop*depth)
		self.lhs.display( nt, ft, depth+1 )
		self.rhs.display( nt, ft, depth+2 )
		#print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))

class Car( Expr ) :
  def __init__(self, thing) :
		self.thing=thing

  def eval( self, nt, ft ) :
#    cList= self.thing.eval(nt,ft);
#
#    if type(self.thing) == type(9):
#      return
    cList=[]
    if type(self.thing) == type([]):
      cList= self.thing[0].eval(nt,ft)
      cList.reverse()
      return cList.pop()
    else:
      cList= self.thing.eval(nt,ft)
      if len(cList)==1:
        return
      return cList[0]


  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
    #print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))

class Cdr( Expr ) :
  def __init__(self, thing) :
    self.thing=thing

  def eval( self, nt, ft ) :
    cList=[]
    if type(self.thing) == type([]):
      print self.thing
      cList= self.thing[0].eval(nt,ft)
      temp=[]
      for i in range(1,len(cList)):
        temp.append(cList[i])
      return temp
    else:
      cList= self.thing.eval(nt,ft)
      temp=[]
      for i in range(1,len(cList)):
        temp.append(cList[i])
      return temp



  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
    #print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))

class Cons( Expr ) :
  def __init__(self, x, y) :
    self.thing=x
    self.thing2=y

  def eval( self, nt, ft ) :
    cList= self.thing2.eval(nt,ft);
    var = self.thing.eval(nt,ft);
    c=var
    d=[]
    for i in range(len(cList)):
      d.append(cList[i])
    d.insert(0,c)
    return d


  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
    #print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))


class ConsFac( Expr ) :
  def __init__(self, thing, thing2) :
    self.thing=thing
    self.thing2=thing2
  
  def eval( self, nt, ft ) :
    cList= self.thing2.eval(nt,ft);
    var = self.thing.eval(nt,ft);
    if type(var) == type([]):
      c=var[0]
    else:
      c=var
    d=[]
    for i in range(len(cList)):
      d.append(cList[i])
    d.insert(0,c)
    return d
    
    
  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
    #print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))

class Null( Expr ) :
  def __init__(self, thing) :
    self.thing=thing

  def eval( self, nt, ft ) :
    cList= self.thing.eval(nt,ft);
#    print cList
    if type(cList) == type([]) and len(cList)==1:
      a=[]
      for i in range(1,len(cList[0])):
        a.append(cList[0][i])
      if (len(cList))>0:
        return 1
      else:
        return 0
    elif type(cList) == type([]):
      if (len(cList))>0:
        return 1
      else:
        return 0
    else:
      print "Error not list object"

  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
    #print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))

class List1( Expr ) :
  def __init__(self, thing) :
    self.thing=thing

  def eval( self, nt, ft ) :
    cList= self.thing.eval(nt,ft);
    if type(cList) == type([]):
        return 0
    else:
        return 1


  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
    #print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))

class Int1( Expr ) :
  def __init__(self, thing) :
    self.thing=thing

  def eval( self, nt, ft ) :
    cList= self.thing.eval(nt,ft); 
    if type(cList) == type(8):
        return 0
    else:
        return 1


  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
    #print "%s= %i" % (tabstop*depth, self.eval( nt, ft ))


class Empty_List( Expr ) :
  def __init__(self) :
    self.thing=[]

  def eval( self, nt, ft ) :
    return self.thing
	
  def null1(self,thing):
    if type(self.thing) == type([]):
      if (len(self.thing))>0:
        return 1
      else:
        return 0
       
class Proc :
	'''Procedural Calls'''

	def __init__( self, List, body ) :
		self.parList = List
		self.body = body

	def apply( self, nt, ft, args ) :
		newContext = {}
		for i in range( len( args )) :
			newContext[ self.parList[i] ] = args[i].eval( nt, ft )
			
		self.body.eval( newContext, ft )
		if newContext.has_key( returnSymbol ) :
			return newContext[ returnSymbol ]
		else :
			print "Error:  no val in Proc returnSymbol not found"
			sys.exit( 2 )
	
	def display( self, nt, ft, depth=0 ) :
		print "%sproc %s :" % (tabstop*depth, str(self.parList))
		self.body.display( nt, ft, depth+1 )



class List( Expr):
  def __init__( self, thing ) :
        self.thing = [thing]

  def eval( self, nt, ft ) :
    for i in range(len(self.thing)):
      self.thing[i]=self.thing[i].eval(nt, ft)
    self.thing.reverse()
    return self.thing

  def append(self, thing2) :
    self.thing.append(thing2)


  def null1(self,thing):
    if type(self.thing) == type([]):
      if (len(self.thing))>0:
        return 1
      else:
        return 0


  def display( self, nt, ft, depth=0 ) :
    print "%sMULT" % (tabstop*depth)
    self.lhs.display( nt, ft, depth+1 )
    self.rhs.display( nt, ft, depth+2 )
  


class FunCall( Expr ) :
	'''functioncall storing, FT'''
	
	def __init__( self, name, argList ) :
		self.name = name
		self.argList = argList
	
	def eval( self, nt, ft ) :
		return ft[ self.name ].apply( nt, ft, self.argList )

	def display( self, nt, ft, depth=0 ) :
		print "%sFC made: %s, args:" % (tabstop*depth, self.name)
		for e in self.argList :
			e.display( nt, ft, depth+1 )



class Stmt :
	'''Statement'''

	def __init__( self ) :
		raise NotImplementedError(
			'Stmt: pure virtual base class.  Do not instantiate' )

	def eval( self, nt, ft ) :
		'''Evaluate the statement'''

		raise NotImplementedError(
			'Stmt.eval: virtual method.  Must be overridden.' )

	def display( self, nt, ft, depth=0 ) :
		'For debugging.'
		raise NotImplementedError(
			'Stmt.display: virtual method.  Must be overridden.' )


class AssignStmt( Stmt ) :
	'''A = 3 any one?'''

	def __init__( self, name, rhs ) :
		'''initialize'''
		self.name = name
		self.rhs = rhs
	
	def eval( self, nt, ft ) :
		nt[ self.name ] = self.rhs.eval( nt, ft )

	def display( self, nt, ft, depth=0 ) :
		print "%sAssign: %s :=" % (tabstop*depth, self.name)
		self.rhs.display( nt, ft, depth+1 )


class DefineStmt( Stmt ) :
	'''Binds a proc object to a name'''

	def __init__( self, name, proc ) :
		self.name = name
		self.proc = proc

	def eval( self, nt, ft ) :
		ft[ self.name ] = self.proc

	def display( self, nt, ft, depth=0 ) :
		print "%sDEFINE %s :" % (tabstop*depth, self.name)
		self.proc.display( nt, ft, depth+1 )


class IfStmt( Stmt ) :

	def __init__( self, cond, tBody, fBody ) :
		'''self, expression ,statement, statement'''
		
		self.cond = cond
		self.tBody = tBody
		self.fBody = fBody

	def eval( self, nt, ft ) :
		if self.cond.eval( nt, ft ) > 0 :
			self.tBody.eval( nt, ft )
		else :
			self.fBody.eval( nt, ft )

	def display( self, nt, ft, depth=0 ) :
		print "%sif" % (tabstop*depth)
		self.cond.display( nt, ft, depth+1 )
		print "%sthen" % (tabstop*depth)
		self.tBody.display( nt, ft, depth+1 )
		print "%selse" % (tabstop*depth)
		self.fBody.display( nt, ft, depth+1 )


class WhileStmt( Stmt ) :

	def __init__( self, cond, body ) :
		'''white statement  self, expression, statement'''
		self.cond = cond
		self.body = body

	def eval( self, nt, ft ) :
		while self.cond.eval( nt, ft ) > 0 :
			self.body.eval( nt, ft )

	def display( self, nt, ft, depth=0 ) :
		print "%sWHILE" % (tabstop*depth)
		self.cond.display( nt, ft, depth+1 )
		print "%sDO" % (tabstop*depth)
		self.body.display( nt, ft, depth+1 )

class StmtList :
	'''self explanatory'''

	def __init__( self ) :
		self.sl = []
	
	def insert( self, stmt ) :
		self.sl.insert( 0, stmt )
	
	def eval( self, nt, ft ) :
		for s in self.sl :
			s.eval( nt, ft )
	
	def display( self, nt, ft, depth=0 ) :
		print "%sSTMT LIST" % (tabstop*depth)
		for s in self.sl :
			s.display( nt, ft, depth+1 )



class Program :
	
	def __init__( self, stmtList ) :
		self.stmtList = stmtList
		self.nameTable = {}
		self.funcTable = {}
	
	def eval( self ) :
		self.stmtList.eval( self.nameTable, self.funcTable )
	
	def dump( self ) :
		print "Dump of Symbol Table"
		print "Name Table"
		for k in self.nameTable :
			print "  %s -> %s " % ( str(k), str(self.nameTable[k]) )
		print "Function Table"
		for k in self.funcTable :
			print "  %s" % str(k)

	def display( self, depth=0 ) :
		print "%sPROGRAM :" % (tabstop*depth)
		self.stmtList.display( self.nameTable, self.funcTable )



n = Plus( Number(17), Number(25) )
l = [ n ]
nt = {}
ft = {}
result = [ i.eval( nt, ft ) for i in l ]

print result

